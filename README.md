...
Eliteserien Fantasy fantasy game data, fetched directly from the official fantasy API at
`https://en.fantasy.eliteserien.no/api/`.
...
.
├── .github/workflows/
│   └── eliteserien-fetch.yml        # scheduled daily fetch (1am UTC, retries 2am/3am)
├── scripts/
...
│       ├── eliteserien-bootstrap_{season}.json
│       ├── eliteserien-fixtures_{season}.json
│       ├── eliteserien-dream-team_{season}.json
│       ├── eliteserien-regions_{season}.json
...
└── latest/                          # always the most recent data, season-agnostic
    ├── eliteserien-bootstrap.json
    ├── eliteserien-fixtures.json
...
| File | Description |
|---|---|
| `data/{season}/eliteserien-bootstrap_{season}.json` | Players, teams, gameweeks, game settings |
| `data/{season}/eliteserien-fixtures_{season}.json` | All season fixtures with scores |
...
| `data/{season}/eliteserien-dream-team_{season}.json` | Season dream team (updated each GW) |
| `data/{season}/eliteserien-set-piece-notes_{season}.json` | Set piece taker notes (if the league API provides them) |
| `data/{season}/eliteserien-regions_{season}.json` | Region/nationality reference (if the league API provides it) |
...
| `data/{season}/csv/teams.csv` | All teams listed in the bootstrap (16 compete in Eliteserien; the platform may list extra historical entries) |
...
| File | Description |
|---|---|
| `latest/eliteserien-bootstrap.json` | Current bootstrap |
| `latest/eliteserien-fixtures.json` | Current fixtures |
...
# Latest bootstrap
url = "https://raw.githubusercontent.com/TopMarx/eliteserien/main/latest/eliteserien-bootstrap.json"
...
```
https://raw.githubusercontent.com/TopMarx/eliteserien/main/data/{season}/eliteserien-bootstrap_{season}.json
https://raw.githubusercontent.com/TopMarx/eliteserien/main/latest/eliteserien-bootstrap.json
```

### Clone the repo

```
git clone https://github.com/TopMarx/eliteserien.git
```
